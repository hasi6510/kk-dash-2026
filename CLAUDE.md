...
## 🔒 恒久ルール（2026-06-13 たっくーさん決定）

翌月以降の予算を設定するときは、**必ず以下を守ること**：

| 項目 | 設定値 | 備考 |
|------|--------|------|
| `income_monthly` | **¥220,000** | 実際の給料が多少前後しても¥220,000で固定（保守的に見積もる） |
| `趣味・自己投資` budget | **¥20,000** | **絶対上限。** たっくーさんの強い意志による設定。増額の相談をされても一度立ち止まって確認すること |

- ボーナスは月収に含めない（入った時はその都度、置き場所を相談する）
- 趣味・自己投資の超過が見えたら早めに声をかける
...
```json
{
  ...
  "variable_spending": [
    ...
    { "name": "被服費",         "actual": 000000 },
    { "name": "ふるさと納税",   "actual": 000000 },
